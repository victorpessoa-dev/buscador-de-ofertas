...
export async function convertAffiliateLink(rawUrl) {
    ...
    if (!hasAffiliate || extractedId !== expectedId) {
        logAffiliateConversion("error", "Não foi possível garantir o ID de afiliado no link final", {
            platform,
            hasAffiliateParams: hasAffiliate,
        });
        throw new Error(`Não foi possível garantir o ID de afiliado da ${PLATFORM_LABELS[platform]} no link final.`);
    }
    return {
        platform,
